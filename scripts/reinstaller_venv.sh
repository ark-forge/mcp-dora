#!/usr/bin/env bash
# Reinstall the production venv from the hash lock (requirements.txt), keep the old one, probe, roll back on failure.
...
set -euo pipefail
REPO_DIR="${REPO_DIR:-/opt/claude-ceo/workspace/mcp-servers/dora}"
SERVICES=(mcp-dora)
PORTS=(8091)
